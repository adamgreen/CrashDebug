...
{
    printf("Usage: CrashDebug --bin imageFilename baseAddress --dump dumpFilename\n"
           "Where: --bin is the required name of the image loaded into the device's FLASH when the crash occurred.\n"
           "         These binary images are typically generated by running\n"
...
static void loadImageFile(CrashDebugCommandLine* pThis)
{
    ...
    __try
    {
        ...
        MemorySim_CreateRegion(pThis->pMemory, pThis->baseAddress, fileSize);
        MemorySim_LoadFromFlashImage(pThis->pMemory, pThis->baseAddress, pBuffer, fileSize);
